...
var resetFileTmpl = template.Must(template.New("file").Parse(`// Code generated by reset; DO NOT EDIT.
...
func (g *generator) generatePackage(pkg packageData) error {
	...
	for _, si := range pkg.structs {
		...
		for _, field := range si.Fields {
			...
			for _, name := range field.Names {
				code := g.fieldResetCode(md.Receiver+"."+name.Name, field.Type, pkg.importPath, importMap)
				if code != "" {
					md.Lines = append(md.Lines, code)
				}
			}
		}
		data.Structs = append(data.Structs, md)
	}

	var buf bytes.Buffer
	if err := resetFileTmpl.Execute(&buf, data); err != nil {
		return err
	}
	...
	g.Logger.Info("generated", zap.String("file", genFile))
	return nil
}
